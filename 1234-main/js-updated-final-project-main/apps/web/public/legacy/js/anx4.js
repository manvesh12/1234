...
function deleteTableBlock(btn) {
  ...
  if (card === container.querySelector('.table-block-card')) {
    toast("You cannot delete the original table.", "warn");
    return;
  }
...
function updateDeleteButtonsVisibility(type) {
  ...
  cards.forEach((card, index) => {
    ...
    if (delBtn) {
      delBtn.style.display = index === 0 ? "none" : "inline-flex";
    }
  });
}
function initRoutesTable() {
  const container = document.getElementById("individual-routes-container");
  if (!container) return;
  // Do not rebuild a populated container on navigation: cloned tables are
  // independent user data and must survive a return to Annexure IV.
  if (!container.querySelector('.table-block-card')) addRouteTableBlock(true);
}
function initClustersTable() {
  const container = document.getElementById("cluster-routes-container");
  if (!container) return;
  if (!container.querySelector('.table-block-card')) addClusterTableBlock(true);
}
