...
/* ══════════════════════════════════════
   TABLE BLOCK CLONING
   ══════════════════════════════════════ */
window.anx3BlockCounter = 0;
function addAnx3TableBlock(sectionType) {
  var containerId = sectionType === 'CLUSTER' ? 'anx3-clusters-container' : 'anx3-contiguous-container';
  var container = document.getElementById(containerId);
  if (!container) return;
  var template = container.querySelector('.anx3-table-block');
  if (!template) return;
  window.anx3BlockCounter++;
  var clone = template.cloneNode(true);
  var table = clone.querySelector('table');
  var oldId = table.id;
  var newId = oldId + '-' + window.anx3BlockCounter;
  table.id = newId;
  ...
  if (addBtn) {
    addBtn.setAttribute('onclick', 'addAnx3CloneRow("' + newId + '", "' + sectionType + '")');
  }
  ...
  if (footer) {
    ...
    removeBtn.onclick = function () { deleteAnx3TableBlock(this); };
    footer.appendChild(removeBtn);
  }
  container.appendChild(clone);
  if (window.initLucide) window.initLucide();
}
window.addAnx3TableBlock = addAnx3TableBlock;
function deleteAnx3TableBlock(btn) {
  var block = btn.closest('.anx3-table-block');
  if (!block) return;
  var container = block.closest('.anx3-table-container');
  var isFirst = container ? block === container.querySelector('.anx3-table-block:first-child') : false;
  if (isFirst) { toast('Cannot remove the first table block.', 'warn'); return; }
  block.remove();
  if (window.initLucide) window.initLucide();
}
window.deleteAnx3TableBlock = deleteAnx3TableBlock;
function addAnx3CloneRow(tableId, sectionType) {
...
window.addAnx3CloneRow = addAnx3CloneRow;
