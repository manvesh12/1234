  ...
  sections.forEach((sec) => {
    ...
    tables.forEach((table, tIdx) => {
      ...
      if (tIdx === 0) {
        ...
        doc.setFont('times', 'bold');
        doc.setFontSize(7.5);
        doc.text('> (contd. Table ' + (tIdx + 1) + ')', 60, startY);
        startY += 14;
      }
...
/* ══════════════════════════════════════
   TABLE BLOCK CLONING
   ══════════════════════════════════════ */
window.anx6BlockCounter = 0;
function addAnx6TableBlock(sectionType) {
  var containerId = sectionType === 'CLUSTER' ? 'anx6-clusters-container' : 'anx6-contiguous-container';
  var container = document.getElementById(containerId);
  if (!container) return;
  var template = container.querySelector('.anx6-table-block');
  if (!template) return;
  window.anx6BlockCounter++;
  var clone = template.cloneNode(true);
  var table = clone.querySelector('table');
  var oldId = table.id;
  var newId = oldId + '-' + window.anx6BlockCounter;
  table.id = newId;
  var tbody = clone.querySelector('tbody');
  var tfoot = clone.querySelector('tfoot');
  if (tbody) tbody.innerHTML = '';
  if (tfoot) tfoot.innerHTML = '';
  var isCluster = sectionType === 'CLUSTER';
  var colCount = isCluster ? 8 : 9;
  var defaultTr = document.createElement('tr');
  for (var c = 0; c <= colCount; c++) {
    var td = document.createElement('td');
    if (c === colCount) {
      td.innerHTML = '<button class="btn btn-xs btn-danger" onclick="delRow(this)" style="display:inline-flex;align-items:center;justify-content:center;padding:4px;"><i data-lucide="trash-2" style="width:12px;height:12px;"></i></button>';
    } else if ((isCluster && c === 3) || (!isCluster && c === 4)) {
      var sel = document.createElement('select');
      sel.innerHTML = '<option>Riverbed</option><option>Patta Land</option>';
      td.appendChild(sel);
    } else {
      td.setAttribute('contenteditable', 'true');
      td.textContent = 'NA';
    }
    defaultTr.appendChild(td);
  }
  if (tbody) tbody.appendChild(defaultTr);
  var addBtn = clone.querySelector('.section-footer .btn-outline');
  if (addBtn) {
    addBtn.setAttribute('onclick', 'addAnx6CloneRow("' + newId + '", "' + sectionType + '")');
  }
  var footer = clone.querySelector('.section-footer');
  if (footer) {
    var removeBtn = document.createElement('button');
    removeBtn.className = 'btn btn-xs btn-danger';
    removeBtn.style.cssText = 'display:inline-flex;align-items:center;gap:6px;margin-left:8px;';
    removeBtn.innerHTML = '<i data-lucide="trash-2" style="width:12px;height:12px;"></i> <span>Remove Table</span>';
    removeBtn.onclick = function () { deleteAnx6TableBlock(this); };
    footer.appendChild(removeBtn);
  }
  container.appendChild(clone);
  if (window.initLucide) window.initLucide();
}
window.addAnx6TableBlock = addAnx6TableBlock;
function deleteAnx6TableBlock(btn) {
  var block = btn.closest('.anx6-table-block');
  if (!block) return;
  var container = block.closest('.anx6-table-container');
  var isFirst = container ? block === container.querySelector('.anx6-table-block:first-child') : false;
  if (isFirst) { toast('Cannot remove the first table block.', 'warn'); return; }
  block.remove();
  if (window.initLucide) window.initLucide();
}
window.deleteAnx6TableBlock = deleteAnx6TableBlock;
function addAnx6CloneRow(tableId, sectionType) {
...
window.addAnx6CloneRow = addAnx6CloneRow;
