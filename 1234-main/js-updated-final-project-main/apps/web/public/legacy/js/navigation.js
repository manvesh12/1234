...
function removeGenericAnnexureTableBlock(button) {
  const block = button?.closest?.('[data-core-annexure-clone="true"]');
  if (!block) return;
  if (!confirm('Are you sure you want to delete this entire table block?')) return;
  const viewId = getViewIdForAnnexureElement(block);
  block.remove();
  if (window.debouncedSaveState) window.debouncedSaveState();
  refreshCoreAnnexurePreview(viewId);
}
function addGenericAnnexureRow(table, viewId, rowTemplate = null) {
  const tbody = table ? table.querySelector('tbody') : null;
  if (!tbody) return;
  const templateRow = rowTemplate || tbody.rows[tbody.rows.length - 1];
  ...
  for (let i = 0; i < columnCount; i++) {
    ...
    if (sourceCell) {
      td.className = sourceCell.className;
      td.style.cssText = sourceCell.style.cssText;
      Array.from(sourceCell.attributes).forEach(attribute => {
        if (attribute.name.startsWith('data-')) td.setAttribute(attribute.name, attribute.value);
      });
    }
...
function addGenericAnnexureRowFromButton(btn) {
  const table = btn?.closest?.('.annexure-f-table-block, .annexure-k-table-block, .table-block-card, .anx-section')?.querySelector('table');
  if (!table) return;
...
function prepareGenericClonedTableBlock(block, sourceTable, newTable, viewId) {
  ...
  block.dataset.coreAnnexureClone = 'true';
  block.querySelectorAll('.anx-table-live-actions').forEach(el => el.remove());
  [block, ...block.querySelectorAll('[data-live-controls-attached]')].forEach(el => {
    if (el.dataset) delete el.dataset.liveControlsAttached;
    el.removeAttribute?.('data-live-controls-attached');
  });
  ...
  addButtons.forEach(button => {
    const onclick = button.getAttribute('onclick') || '';
    const usesNewTableId = inlineHandlerReferencesId(onclick, newTable?.id);
    const usesSourceTable = inlineHandlerReferencesId(onclick, sourceTable?.id);
    const usesDynamicRef = /\(this[,)]/.test(onclick) || /\(btn[,)]/.test(onclick) || /\(event[,)]/.test(onclick);
    if (usesSourceTable || (!usesNewTableId && !usesDynamicRef)) {
  ...
  block.querySelectorAll('.rm-sec-a-btn, .rm-sec-btn').forEach(btn => {
    ...
    btn.setAttribute('onclick', 'window.removeGenericAnnexureTableBlock(this)');
    deleteButtonFound = true;
  });
  if (!deleteButtonFound) {
    var footer = block.querySelector('.section-footer');
    if (footer && !footer.querySelector('.rm-sec-a-btn, .rm-sec-btn')) {
      var hasRemoveOnclick = Array.from(footer.querySelectorAll('button[onclick]')).some(function(b) { return (b.getAttribute('onclick') || '').toLowerCase().indexOf('remove') !== -1; });
      if (!hasRemoveOnclick) {
      var delBtn = document.createElement('button');
      delBtn.type = 'button';
      delBtn.className = 'btn btn-xs btn-danger';
      delBtn.style.cssText = 'display:inline-flex; align-items:center; gap:6px;';
      delBtn.setAttribute('onclick', 'window.removeGenericAnnexureTableBlock(this)');
      delBtn.innerHTML = '<i data-lucide=\"trash-2\" style=\"width:12px;height:12px;\"></i><span>Remove This Table</span>';
      footer.appendChild(delBtn);
      }
    }
  }
  // Match Annexure F: a clone keeps the table structure but begins with its
  // own empty row, rather than sharing a copy of the source table's data.
  const tbody = newTable.querySelector('tbody');
  const rowTemplate = sourceTable?.querySelector('tbody tr:last-child');
  if (tbody) tbody.replaceChildren();
  addGenericAnnexureRow(newTable, viewId, rowTemplate);
  if (window.initLucide) window.initLucide();
...
function addGenericTableBlock(btn) {
  const sourceBlock = btn?.closest?.('.anx-section, .anx3-table-block, .anx6-table-block, .table-block-card');
...
window.removeGenericAnnexureTableBlock = removeGenericAnnexureTableBlock;
...
function ensureGenericTableCloneButtons(view) {
  view?.querySelectorAll?.('.anx-section').forEach(section => {
    // Annexures F and K already own their section-aware clone factories.
    // Do not add a generic button around those multi-table containers.
    if (!section.querySelector('table') ||
      section.querySelector('.annexure-f-table-block, .annexure-k-table-block') ||
      section.querySelector('button[onclick*="addGenericTableBlock"]')) return;
...
function addCoreAnnexureTableControls(id) {
  ...
  // The first table in every section is the non-deletable anchor.  Only
  // prepareGenericClonedTableBlock marks a cloned section as removable.
  view.querySelectorAll('.anx-section:not([data-core-annexure-clone="true"]) .rm-sec-a-btn, .anx-section:not([data-core-annexure-clone="true"]) .rm-sec-btn').forEach(button => {
